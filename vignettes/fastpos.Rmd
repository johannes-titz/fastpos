...
<!-- Note that because we use two rhos, the simulation is parallelized (at least under GNU/Linux), but we also loose the progress bar. If it takes too long, you can stop the simulation by pressing Escape. -->

If you want to dig deeper, you can have a look at the functions that fastpos builds upon. **simulate_pos** is the workhorse of the package. It calls a C++ function to calculate correlations sequentially and it does it pretty fast (but you know that already, right?). A rawish approach would be to create a population with **create_pop** and pass it to **simulate_pos**:
